...
static char MessageBack[BUF_LEN] = "--roll\n";
...
static ssize_t device_read(struct file *fp, char *buff, size_t length, loff_t *offset) {
	...
	if(DirectionOutput){
		...
		while (length && *buff_rptr) {
			put_user(*(buff_rptr++), buff++);
			length--;
			bytes_read++;
		}
	}
		...
		while (length && *MessageBuf_Ptr) {
			put_user(*(MessageBuf_Ptr++), buff++);
			length--;
			if (CurrentSymbOut <= strlen(MessageBuf))
				bytes_read++;
			else
				bytes_read--;
...
static ssize_t device_write(struct file *fp, const char *buff, size_t length, loff_t *offset) {
	...
	if(strstr(MessageTemp, MessageDelete) != 0){
		printk("5" "chardev: deleted. \n");
		copy_from_user(buff_wptr, 0, bytes_written);
		buff_wptr += bytes_written;
	}
	else{
		if(strstr(MessageTemp, MessageBack) != 0){
			printk("5" "chardev: back flag = %i\n",DirectionOutput);
			DirectionOutput = !DirectionOutput;
			if(bytes_written > length) bytes_written = length;

		}
		else {
			printk("5" "chardev: set message. %i\n", strstr(MessageTemp, MessageBack));
			if(bytes_written > length) bytes_written = length;
